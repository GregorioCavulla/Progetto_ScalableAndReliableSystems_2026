...
from agent_demo import run_coordinator
...
def snapshot(mcp: DroneMCP) -> dict:
    ...
    observer_last = _read_json_file(data_dir / "last_observer_run.json")
    remediation_last = _read_json_file(data_dir / "last_remediation_run.json")
    ...
    return {
        ...
        "last_observer_result": observer_last,
        "last_remediation_result": remediation_last,
    }
...
<body>
  <div class="wrap">
    <h1>DroneV1 Dashboard</h1>
    <div class="sub">Two-role agent architecture: Observer + Remediation</div>
    ...
    <div class="card" style="margin-top: 12px;">
      <div class="title">Coordinator Agent</div>
      <div id="decision" class="mono">-</div>
      <div id="observer_decision" class="mono" style="margin-top:8px; color:#334e68;">observer: -</div>
      <div id="remediation_decision" class="mono" style="margin-top:4px; color:#334e68;">remediation: -</div>
      <div class="actions">
    ...
    <div class="row">
      <div class="card">
        ...
        <table>
          <thead><tr><th>drone</th><th>status</th><th>battery</th><th>wear</th><th>charging</th><th>delivering</th><th>position</th><th>wind</th></tr></thead>
  ...
  <script>
    ...
    async function loadSnapshot() {
      ...
      document.getElementById('decision').textContent = (data.last_agent_result || {}).final_decision_text || 'agent_not_run';
      document.getElementById('observer_decision').textContent = 'observer: ' + ((data.last_observer_result || {}).observer_summary || 'n/a');
      document.getElementById('remediation_decision').textContent = 'remediation: ' + ((data.last_remediation_result || {}).remediation_summary || 'n/a');
      ...
      const trace = {
        observer_tool_trace: (data.last_observer_result || {}).observer_tool_trace || [],
        remediation_tool_trace: (data.last_remediation_result || {}).remediation_tool_trace || []
      };
        ...
        def do_GET(self):  # noqa: N802
            ...
            if parsed.path == "/api/run_agent":
                try:
                    ...
                    snapshot.last_agent_result = {
                        "final_decision_text": "coordinator_completed",
                        "tool_trace": [],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="DroneV1 dashboard")
...
def main() -> None:
    ...
    print(f"DroneV1 dashboard on http://{args.host}:{args.port}")
