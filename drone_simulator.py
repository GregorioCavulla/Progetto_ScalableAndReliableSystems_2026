...
class Drone:
    def __init__(self, drone_id):
        ...
        self.returning_start_time = None # Timer per il ritorno alla base
    ...
    def handle_command(self, payload):
        ...
        try:
            ...
            if command == "assign_mission" and self.state == "IDLE":
                ...
                self.current_order = None
                self.returning_start_time = time.time()  # Inizia il timer di ritorno
    ...
    def update(self):
            ...
            if self.target_lat and self.target_lon:
                ...
                if distance < self.speed:
                    ...
                    if self.state == "IN_DELIVERY":
                        ...
                        self.target_lat = BASE_LAT
                        self.target_lon = BASE_LON
                        self.returning_start_time = time.time()  # Inizia il timer
                    else:
        ...
        # Verifica timeout per il ritorno a base (FIX BUG: evita che il drone rimanga in RETURNING per sempre)
        if self.state == "RETURNING" and self.returning_start_time:
            elapsed = time.time() - self.returning_start_time
            if elapsed > 30:  # Se il drone è in RETURNING per più di 30 secondi, diventa IDLE
                print(f"[{self.id}]  Timeout ritorno a base - forzato passaggio a IDLE.")
                self.state = "IDLE"
                self.target_lat = None
                self.target_lon = None
                self.returning_start_time = None
