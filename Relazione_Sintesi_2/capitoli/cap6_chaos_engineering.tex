...
La suite \texttt{ops/chaos\_test\_suite.py} inietta guasti controllati sui componenti critici (database, broker, capacità di flotta) per misurare la reazione del sistema in termini di RTO e capacità di auto-rimediazione. L'obiettivo non è dimostrare l'assenza di guasti, ma misurare empiricamente la \textit{reazione} dell'architettura discussa nel Capitolo~\ref{cap:reliability_scalability}.

\section{Metodologia}
\label{sec:metodologia_chaos}

Ogni esperimento segue il ciclo classico del chaos engineering:
\begin{enumerate}
    \item \textbf{Ipotesi:} comportamento atteso e SLO da rispettare.
    \item \textbf{Blast radius:} guasto confinato a un singolo Deployment in namespace \texttt{default}; il control-plane non viene mai toccato.
    \item \textbf{Iniezione:} via API Kubernetes (\texttt{patch\_namespaced\_deployment\_scale}, \texttt{delete\_namespaced\_pod}) o tramite generatore di carico (\texttt{client\_simulator.py --stress}).
    \item \textbf{Osservazione:} metriche su \texttt{chaos\_test\_results.log} (timestamp, RTO, repliche prima/dopo).
